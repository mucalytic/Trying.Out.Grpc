...
public class FirstService : FirstServiceDefinition.FirstServiceDefinitionBase
{
    ...
    {
        context.ResponseTrailers.Add(new Metadata.Entry("do-not-forget", "you-are-awesome"));
        var header = context.RequestHeaders.Get("purpose");
        if (header is not null)
        {
            await responseStream.WriteAsync(new Response { Message = $"Your purpose is {header.Value} " });
        }
        for (var i = 0; i < 10; i++)
        {
